...
// Handler returns the handler that was registered in the router.
func (ctx *context) Handler() Handler {
	return ctx.server.router.LookupNoAlloc(ctx.request.method, ctx.request.path, ctx.request.addParameter)
}

// Next executes the next handler in the middleware chain.
func (ctx *context) Next(next Context) error {
	ctx.handlerCount++
	return ctx.server.handlers[ctx.handlerCount](next)
}
